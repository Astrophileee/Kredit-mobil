    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col">
                ...
                @endif
                <form action="{{ route('cash.update',$cash->kode_cash ) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    <div class="card">
                        <div class="card-body">
                          <div class="form-group">
                            <label for="title">Kode Cash:  {{ $cash->kode_cash }}</label>
                            <input type="hidden" class="form-control" id="kode_cash" name="kode_cash"
                                placeholder="kode Mobil" value="{{ old('kode_cash') ?? $cash->kode_cash }}">       
                        </div>
                        <div class="form-group">
                            <label>Kode Mobil</label>
                                <select name="kode_mobil" style="width: 100%" class="form-control">
                                    @foreach ($cars as $car)
                                    <option value="{{ $car->kode_mobil }}">{{ $car->nama }}</option>
                                    @endforeach
                                </select>
                        </div>
                        <div class="form-group">
                            <label>No Ktp Pembeli</label>
                                <select name="ktp_pembeli" style="width: 100%" class="form-control">
                                    @foreach ($buyers as $buyer)
                                    <option value="{{ $buyer->ktp_pembeli }}">{{ $buyer->nama_pembeli }}</option>
                                    @endforeach
                                </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control" id="tgl_beli_cash" name="tgl_beli_cash"
                                placeholder="Tanggal Beli Cash" value="{{ old('tgl_beli_cash') ?? $cash->tgl_beli_cash}}">
                        </div>
                        <div class="form-group">
                            <label for="bayar_cash">Bayar Cash</label>
                            <input type="number" class="form-control" id="bayar_cash" name="bayar_cash"
                                placeholder="Bayar Cash" value="{{ old('bayar_cash') ?? $cash->bayar_cash}}">
                        </div>
